...
impl Bot {
    ...
    pub fn get_move(&self, board: Board) -> ChessMove {
        ...
        let (pos_score, best_move) =
            self.negamax_ab(board, depth, -INFTY, INFTY, self.objective); //self.negamax(board, depth, self.objective);
...
fn evaluate(board: &Board) -> i32 {
    ...
    let mut mat_white = pawns_w * pawn + knights_w * knight + bishops_w * bishop + rooks_w * rook + queens_w * queen;
    let mut mat_black = pawns_b * pawn + knights_b * knight + bishops_b * bishop + rooks_b * rook + queens_b * queen;
    ...
    let endgame_force_king = king_corner_score_w * endgame_factor_b * 2 / pawn - king_corner_score_b * endgame_factor_w * 2 / pawn;
    ...
    mat_score + endgame_force_king + mobility
}


fn eval_from_fen(fen: &str) -> i32 {
